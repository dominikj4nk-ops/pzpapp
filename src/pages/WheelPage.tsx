...
const steps = [
  { icon: Download, title: "Ulož si obrázek výhry", text: "Ke každé výhře ti vygenerujeme obrázek na míru — stačí ho uložit do galerie a dát na Instagram story." },
  { icon: Instagram, title: "Označ @prachyzaregistraci", text: "Bez označení tě nemůžeme zařadit do týdenního slosování." },
  { icon: CalendarClock, title: `Každou ${DRAW_DAY} v ${DRAW_TIME} losujeme +${WEEKLY_SHARE_BONUS} Kč`, text: "Ze všech, kdo ten týden sdíleli story, vybereme jednoho. Bonus dostane navíc k tomu, co mu padlo." }
];
...
const recentWinners = [
  { handle: "@anet.spori", when: "minulý týden" },
  { handle: "@kuba_invest", when: "před 2 týdny" },
  { handle: "@terka.fit", when: "před 3 týdny" }
];
...
export default function WheelPage() {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    setStoryImage(null);
    void prizeStoryDataUrl(resultPrize).then((url) => {
  ...
  return (
    <>
      ...
      <section className="space-y-4">
        <section className="wheel-hero relative overflow-hidden rounded-[24px] border border-white/10 p-4 shadow-[0_18px_56px_rgba(0,0,0,.34)] xl:p-5">
          ...
          <div className="relative max-w-[300px]">
            <p className="inline-flex items-center gap-1.5 rounded-full border border-neon/25 bg-neon/10 px-2.5 py-1 text-[10px] font-black uppercase tracking-wide text-neon">
              <FerrisWheel size={12} /> Každý den 1 otočka zdarma
            </p>
            ...
            <p className="mt-2 text-sm leading-6 text-slate-300">
              Zatoč si, výsledek dej na story a označ <span className="font-bold text-neon">@prachyzaregistraci</span>. Každý týden
              losujeme ze všech, kdo sdíleli, a rozdáváme bonus +{WEEKLY_SHARE_BONUS} Kč.
            </p>
          </div>
        </section>
        ...
        <section className="overflow-hidden rounded-[24px] border border-neon/25 bg-neon/[.06] p-4">
          <div className="flex items-start gap-3">
            <span className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-neon/15 text-neon">
              <Gift size={20} />
            </span>
            <div className="min-w-0">
              <h2 className="text-sm font-black text-white">Týdenní bonus +{WEEKLY_SHARE_BONUS} Kč za story</h2>
              <p className="mt-1 text-xs leading-5 text-slate-300">
                Nezáleží, co ti padlo — sdílením hraje každý o stejný bonus. Losujeme každou {DRAW_DAY} v {DRAW_TIME}.
              </p>
              {sharedThisWeek ? (
                <p className="mt-2 inline-flex items-center gap-1.5 rounded-full border border-neon/30 bg-neon/10 px-2.5 py-1 text-[11px] font-black text-neon">
        ...
        <div className="grid gap-2">
          {steps.map((step) => {
            ...
            return (
              <div key={step.title} className="flex items-start gap-3 rounded-[16px] border border-white/10 bg-white/[.035] p-3">
                <span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-white/[.06] text-neon">
                  <Icon size={18} />
                </span>
                <div className="min-w-0">
        ...
        <section>
              ...
              return (
                <div key={winner.handle} className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/[.035] p-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-slate-400">+{WEEKLY_SHARE_BONUS} Kč bonus za story</p>
                  </div>
          ...
          <p className="mt-2 text-[11px] leading-4 text-slate-500">
            Výherce týdenního bonusu vyhlašujeme každou {DRAW_DAY} v {DRAW_TIME} na našem Instagramu.
          </p>
        </section>
      </section>

      {showResult && resultPrize ? (
        <div className="fixed inset-0 z-[60] flex items-center justify-center overflow-hidden bg-black/70 p-5 backdrop-blur-sm">
          ...
          <div
            className="result-pop relative max-h-[92vh] w-full max-w-sm overflow-y-auto rounded-[26px] border border-white/12 p-4 text-center shadow-[0_30px_80px_rgba(0,0,0,.6)]"
            style={{ background: resultBackground(resultPrize.tier) }}
          >
            <button
              onClick={() => setShowResult(false)}
              aria-label="Zavřít"
              className="absolute right-3 top-3 grid h-9 w-9 place-items-center rounded-full bg-black/25 text-white/80 transition hover:bg-black/40"
            >
              <X size={18} />
            </button>

            <p className="text-[11px] font-black uppercase text-white/60">Padlo ti</p>
            <h2 className="mt-1 text-2xl font-black leading-tight text-white">{resultPrize.label}</h2>

            {storyImage ? (
              <img
                src={storyImage}
                alt={`Story obrázek – ${resultPrize.label}`}
                className="mx-auto mt-4 w-full max-w-[250px] rounded-[24px] border border-white/15 shadow-[0_18px_54px_rgba(0,0,0,.42)]"
              />
            ) : (
              <div className="mx-auto mt-4 grid aspect-[9/16] w-full max-w-[250px] animate-pulse place-items-center rounded-[24px] border border-white/10 bg-white/[.05]">
            ...
            <div className="mt-4 rounded-2xl border border-white/15 bg-black/20 p-3 text-left">
              <p className="text-[13px] font-black text-white">Jak se dostat do slosování o +{WEEKLY_SHARE_BONUS} Kč:</p>
              <ol className="mt-2 space-y-1.5 text-xs leading-5 text-white/80">
                ...
                <li>3. Každou {DRAW_DAY} v {DRAW_TIME} losujeme — výherce vyhlásíme na Instagramu a ozveme se mu.</li>
              </ol>
            </div>

            {sharedThisWeek ? (
              <p className="mt-4 inline-flex items-center gap-1.5 rounded-full border border-neon/30 bg-neon/10 px-3 py-1.5 text-xs font-black text-neon">
                <BadgeCheck size={15} /> Tenhle týden jsi ve slosování
              </p>
            ) : null}
            ...
            <button onClick={() => setShowResult(false)} className="mt-3 text-sm font-bold text-white/70 underline-offset-2 hover:underline">
              Zavřít
            </button>
          </div>
        </div>
