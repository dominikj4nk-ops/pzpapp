  ...
  return (
    <>
      ...
      <div className="mx-auto w-full max-w-[1080px] pb-4 xl:max-w-[1320px]">
        <section className="reviews-hero relative overflow-hidden rounded-[28px] border border-white/10 p-6 shadow-[0_24px_70px_rgba(0,0,0,.4)] sm:p-10 xl:p-8 xl:shadow-none">
          <p className="inline-flex items-center gap-1.5 rounded-full border border-neon/25 bg-neon/10 px-3 py-1 text-[11px] font-black uppercase tracking-wide text-neon xl:hidden">
            <BadgeCheck size={13} /> Transparentní metodika
          </p>
          <p className="hidden text-[11px] font-black uppercase text-slate-500 xl:block">Metodika katalogu</p>
          <h1 className="mt-4 max-w-[720px] text-[34px] font-black leading-[1.05] sm:text-[52px] xl:mt-3 xl:text-[46px]">Jak ověřujeme nabídky</h1>
          ...
          {checks.map((check, index) => {
            ...
            return (
              <article key={check.title} className="glass p-5">
                <span className="grid h-11 w-11 place-items-center rounded-2xl bg-neon/12 text-neon xl:hidden"><Icon size={20} /></span>
                <p className="hidden text-[10px] font-black text-neon xl:block">0{index + 1}</p>
                <h2 className="mt-3 text-base font-black xl:mt-2">{check.title}</h2>
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            {bonuses.map((bonus) => (
              <article key={bonus.id} className="glass flex items-center gap-3 p-4">
                <span className="grid h-10 w-10 shrink-0 place-items-center rounded-2xl bg-neon/10 text-neon xl:hidden"><ShieldCheck size={18} /></span>
                <span className="hidden xl:block"><LogoMark bonus={bonus} size="sm" /></span>
