...
export default function AppShell({ children }: AppShellProps) {
  ...
  return (
    <main id="app-scroll-root" className="app-scroll-root desktop-shell bg-[#030b12] text-white">
      <aside className="desktop-sidebar fixed inset-y-0 left-0 z-50 hidden w-[264px] flex-col border-r border-white/10 bg-[#0c1217] xl:flex">
        <div className="flex h-16 shrink-0 items-center border-b border-white/10 px-5">
          <BrandLogo className="text-[18px]" onClick={() => navigate(paths.home)} tone="ink" />
        </div>
        ...
        <div className="dashboard-sidebar-scroll flex min-h-0 flex-1 flex-col overflow-y-auto px-3 py-4">
          ...
          <div className="mt-auto pt-6">
            <div className="border-t border-white/10 px-1 pt-5">
              <p className="text-[9px] font-black uppercase text-slate-600">Aktuální katalog</p>
              <p className="mt-2 text-sm font-bold text-white">{bonuses.length} aktivních nabídek</p>
              <p className="mt-2 text-[11px] leading-5 text-slate-500">U každé uvádíme podmínky a oficiální zdroj.</p>
              <button
      ...
      <div className="xl:ml-[264px] xl:min-w-0">
        <header className="desktop-topbar sticky top-0 z-40 hidden h-16 items-center border-b border-white/10 bg-[#0a0f13] px-7 xl:flex 2xl:px-9">
          <div className="min-w-0">
            <p className="truncate text-base font-black text-white">{pageTitle(location.pathname)}</p>
          </div>
        ...
        <footer className="desktop-footer hidden items-center justify-between border-t border-white/10 px-7 py-5 text-[11px] text-slate-600 xl:flex 2xl:px-9">
